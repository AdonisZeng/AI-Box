...
export function AgentContextRail({
  ...
  return (
    <aside className="flex min-h-0 flex-col gap-3 overflow-y-auto border-t border-slate-200/75 bg-white/46 p-4 backdrop-blur-xl dark:border-slate-700/70 dark:bg-slate-950/22 xl:border-l xl:border-t-0">
      <RailCard>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  active
                    ? 'border-[#4a9eff]/70 bg-[#4a9eff]/12 text-slate-900 shadow-[0_0_0_1px_rgba(74,158,255,0.14)] dark:text-slate-50'
                    : 'border-slate-200/80 bg-white/54 text-slate-500 hover:border-slate-300 hover:text-slate-800 dark:border-slate-700/70 dark:bg-slate-900/42 dark:text-slate-400 dark:hover:border-slate-600 dark:hover:text-slate-100'
                )}
              >
        ...
        {plan.length > 0 ? (
          <ol className="mt-3 space-y-2">
            {plan.map((step, index) => (
              <li
                key={`${step}-${index}`}
                className="flex gap-2 rounded-2xl border border-slate-200/70 bg-white/54 px-3 py-2 text-xs leading-5 text-slate-600 dark:border-slate-700/70 dark:bg-slate-900/42 dark:text-slate-300"
              >
        ...
        {selectedSkills.length > 0 ? (
          <div className="mt-3 flex flex-wrap gap-2">
            {selectedSkills.map((skill, index) => (
              <span
                key={`${skill}-${index}`}
                className="rounded-full border border-sky-200/80 bg-sky-50/80 px-3 py-1 text-xs font-medium text-sky-700 dark:border-sky-900/70 dark:bg-sky-950/42 dark:text-sky-200"
              >
                {skill}
              </span>
            ))}
...
function RailCard({
  ...
  return (
    <section
      className={cn(
        'rounded-[22px] border border-white/76 bg-white/68 p-4 shadow-[0_14px_34px_rgba(148,163,184,0.12)] backdrop-blur-xl dark:border-slate-700/70 dark:bg-slate-900/48',
        className
      )}
    >
      {children}
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 rounded-2xl border border-slate-200/70 bg-white/54 px-3 py-2 text-xs dark:border-slate-700/70 dark:bg-slate-900/42">
...
function EmptyRailText({ children }: { children: ReactNode }) {
  return (
    <p className="mt-3 rounded-2xl border border-dashed border-slate-200/80 bg-white/42 px-3 py-3 text-xs leading-5 text-slate-400 dark:border-slate-700/70 dark:bg-slate-900/32">
      {children}
    </p>
  )
}
